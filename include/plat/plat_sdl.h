...
{
public:
    GW_PlatformSDL_Image(SDL_Renderer *renderer, const string &filename, GW_Platform_RGB *tcolor);
    virtual ~GW_PlatformSDL_Image();

    virtual int width_get() { return w_; }
    virtual int height_get() { return h_; }

    virtual bool resize_fit(int w, int h);

    SDL_Texture *texture_get() { return texture_; }
private:
    int w_, h_;
...
{
public:
    GW_PlatformSDL(int width, int height, bool fullscreen = false);
    ...
    virtual GW_Platform_Point translate_touch_coordinates(GW_Platform_FPoint* touch);
...
#ifdef GW_DEBUG
    virtual void draw_touch(const char* log, GW_Platform_Point* touch);
#endif
